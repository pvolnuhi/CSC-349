...
class TopSorter  {	
   ...
   //topological sort using dfs removal technique 

	//firsy DFS method implementation



	
   public void readfile_graph(String filename) throws FileNotFoundException  {
      int x,y;
        //read the input
   ...
   public static ArrayList<Integer> srcRemTopSorter(String filename){
   	...
   	try{
   		sort.readfile_graph(filename);
   	}catch (Exception e){
   		System.out.println(e.getMessage());
   	}
  
	ArrayList<Integer> adjList[] = new ArrayList[sort.nvertices];
		for (int i = 0; i < sort.nvertices; i++) {
		...
		ArrayList<Integer> srcremoval = new ArrayList<Integer>();
		...
		while (queue.size() != 0) {
			int vectork = queue.poll();
			srcremoval.add(vectork );

			for (Vertex w : sort.vertices.get(vectork - 1).edges) {
				adjList[w.key - 1].remove((Integer) vectork);
		...
		int negnum = sort.nvertices - srcremoval.size();
   ...
   public static boolean dfs(Vertex vertex, Stack<Vertex> stack) {
    ...
    for (Vertex edge : vertex.edges) {
        if (!edge.processed) {
            ...
            boolean childResult = dfs(edge, stack);
            if (!childResult) {
                return false;
            }
        }

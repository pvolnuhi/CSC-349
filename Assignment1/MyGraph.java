...
public class MyGraph {
  ...
  public ArrayList<HashSet<Integer>> connectCheck() {
    ...
    ArrayList<Vertex> vv = copy(vertices);
    queue.offer(vv.remove(0));

    while (!vv.isEmpty() || !queue.isEmpty()) {
      HashSet<Integer> comp = new HashSet<Integer>();

      while (!queue.isEmpty()) {
        Vertex v = queue.poll();
        if (v.processed) continue;
        ArrayList<Vertex> edges = v.edges;
        for (Vertex ver : edges) {
          if (ver.processed) continue;
          queue.offer(ver);
        }
        v.processed = true;
        comp.add(v.key);
  ...
  ArrayList<Vertex> copy(ArrayList<Vertex> list) {
  ...
  public boolean bipartiteCheck() {
    ...
    while (!vv.isEmpty() || !queue.isEmpty()) {
      ...
      while (!queue.isEmpty()) {
        ...
        ArrayList<Vertex> edges = v.edges;
        for (Vertex ver : edges) {
          if (ver.color == v.color) {
            // not bipartite
            return false;
          } else if (ver.color == -1) {
            ver.color = v.color == 0 ? 1 : 0;
            queue.offer(ver);
          }
        }
        vv.remove(v);
      ...
      // vv list may contains only uncolored vertices now
      // Check if it is not empty, then remove first, color it and
      // put into the queue.
      if (!vv.isEmpty()) {
        Vertex v1 = vv.remove(0);
        v1.color = 0;
        queue.offer(v1);
      }
    }
  ...
  void remove_edge(Vertex x, Vertex y) {
    if (x.degree < 0) System.out.println("Warning: no edge --" + x + ", " + y);
    x.edges.remove(y);
    x.degree--;
  }
